...
function NavBarAdmin() {
  ...
  useEffect(() => {
    ...
    } else if (currentPath.includes('product-dashboard')) {
  ...
  const navItems = [
    ...
    },
    {
      id: 'products',
      label: 'Products',
      href: 'http://localhost:3000/product-dashboard',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
  ...
  return (
    <aside className="fixed top-0 left-0 flex flex-col h-screen text-white border-r shadow-2xl w-60 bg-gradient-to-b from-slate-900 via-slate-950 to-slate-900 border-green-800/20">
      {/* Logo & Brand */}
      <div className="relative flex items-center gap-3 px-6 py-6 mt-8 mb-4">
        ...
        <a href="http://localhost:3000/" className="text-xl font-bold text-transparent transition-all duration-300 hover:text-green-300 bg-gradient-to-r from-white to-green-100 bg-clip-text hover:from-green-300 hover:to-green-200">
          Cyber Seeds
        </a>
      </div>
      ...
      <nav className="flex flex-col gap-4 px-4">
        ...
        <a
          href="#"
          className="relative flex items-center gap-3 px-5 py-3 mt-10 overflow-hidden font-normal transition-all duration-300 rounded-md cursor-pointer group hover:bg-gradient-to-r hover:from-red-800/30 hover:to-red-700/30 hover:text-red-300 hover:shadow-md hover:border hover:border-red-600/30"
        >
